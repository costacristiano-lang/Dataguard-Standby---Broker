# Oracle Data Guard: Post-Implementation Daily Check

[English](./README-oracle-data-guard-daily-check.md) | [Português (Brasil)](./README-oracle-data-guard-daily-check.pt-BR.md)
...
GitHub Markdown guide for daily monitoring after implementing an Oracle Data Guard environment.

## Objective

Use this operational runbook to verify that the `PRIMARY` and `PHYSICAL STANDBY` remain healthy, synchronized, and ready for Disaster Recovery (DR).
...
- [ ] confirm which database is `PRIMARY`
- [ ] confirm which database is `PHYSICAL STANDBY`
- [ ] validate `open_mode` and `database_role`
- [ ] validate redo transport without errors
- [ ] validate redo apply without abnormal delay
- [ ] validate `transport lag` and `apply lag`
- [ ] validate Broker without `WARNING` or `ERROR`
- [ ] validate `MRP` running on the standby
- [ ] validate archive destinations without failures
- [ ] validate FRA and disk-space usage
- [ ] review recent `alert log` errors
- [ ] record the day.s evidence

## 1. Validate Database Role and State
...
## 2. Validate Redo Transport and Apply
...
## 3. Validate Synchronization Lag
...
## 5. Validate Archive Destinations
...
## 6. Validate Managed Recovery Process
...
## 8. Validate Recent Alert Log Errors
...
## 9. Validate Archive and FRA Space
...
## 10. Validate Flashback and DR Readiness
